...
object ShimGeneratorSpec extends ZIOSpecDefault:

    private def tempPath(name: String): os.Path =
        os.Path(java.nio.file.Paths.get("out", "mill-interceptor-tests", name).toAbsolutePath)
    ...
    def spec: Spec[Any, Any] = suite("ShimGenerator")(
        ...
        suite("windowsContent")(
            ...
            test("uses powershell for download") {
                val content = ShimGenerator.windowsContent(BuildTool.Maven, "0.1.0")
                assertTrue(content.contains("powershell"))
            }
        ),
        suite("generate")(
            test("writes shim files to the requested directory") {
                ...
                val unixPath = outputDir / "mvn"
                val cmdPath  = outputDir / "mvn.cmd"

                os.remove.all(outputDir)
                val generated     = ShimGenerator.generate(options)
                val unixExists    = os.exists(unixPath)
                val windowsExists = os.exists(cmdPath)
                val unixContent   = os.read(unixPath)
                val windowsContent = os.read(cmdPath)
                os.remove.all(outputDir)
