...
/** Parses raw Maven CLI arguments into a [[MvnCommand]].
  *
  * Named flags are handled by case-app via [[MvnArgParser.Options]].
  * `-D` system properties use a prefix pattern that case-app cannot
  * express, so they are extracted before case-app sees the args.
  * Remaining positional arguments are classified as lifecycle phases
  * or Maven goals.
  */
object MvnArgParser:
    ...
    /** Maven uses single-dash multi-character flags (e.g. `-pl`, `-am`, `-amd`)
      * which is non-standard. case-app treats multi-char names as long options
      * (requiring `--`). This set lists the Maven flags that need normalising.
      */
    private val mavenShortFlags: Set[String] = Set(
      "-pl", "-am", "-amd"
    )

    /** Normalise Maven's single-dash multi-char flags to double-dash
      * so case-app can recognise them.
      */
    ...
    /** Extract `-Dkey=value` and `-Dkey` arguments, returning the properties
      * and the remaining args with `-D` entries removed.
      */
