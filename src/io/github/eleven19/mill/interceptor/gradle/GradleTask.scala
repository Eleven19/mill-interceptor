...
/** Gradle lifecycle tasks and their mapping to Mill tasks.
  *
  * Each Gradle task maps to one or more [[MillTask]]s that together
  * reproduce the effect of that Gradle task. Tasks are ordered so
  * that earlier prerequisites come first.
  *
  * Unlike Maven's strict lifecycle phases, Gradle tasks are more
  * ad-hoc.  The mapping here covers the standard Gradle lifecycle
  * tasks from the `base`, `java`, and `maven-publish` plugins.
  */
enum GradleTask(val millTasks: List[MillTask]):
    ...
    case PublishToMavenLocal extends GradleTask(List(MillTask("compile"), MillTask("test"), MillTask("jar"), MillTask("publishLocal")))
