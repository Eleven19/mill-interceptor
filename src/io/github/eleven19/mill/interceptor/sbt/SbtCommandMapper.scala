...
/** Maps a parsed sbt command to a sequence of [[MillTask]]s.
  *
  * sbt task names are mapped to Mill tasks; test implies compile first
  * (as in sbt). Order is preserved and duplicates removed.
  */
object SbtCommandMapper:
    ...
    private def sbtTaskToMill(name: String): List[MillTask] =
        name.toLowerCase match
            ...
            case "publishlocal"  => List(MillTask("compile"), MillTask("test"), MillTask("jar"), MillTask("publishLocal"))
